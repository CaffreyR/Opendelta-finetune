...
def main():
    ...
    if delta_args.delta_type.lower() != "none":
        ...
        delta_model = LoraModel.from_finetuned("saved/"+data_args.task_name, backbone_model=model,local_files_only=True)
        #delta_config = AutoDeltaConfig.from_dict(vars(delta_args))
        #delta_model = AutoDeltaModel.from_finetuned("saved/"+data_args.task_name, backbone_model=model,local_files_only=True)
    ...
    if training_args.push_to_hub: # TODO add description here
        ...
        delta_model.save_finetuned(repo_name,push_to_dc=False)
