...
def main():
    ...
    if delta_args.delta_type.lower() != "none":
        from opendelta import AutoDeltaConfig,AutoDeltaModel,LoraModel
        delta_config = AutoDeltaConfig.from_dict(vars(delta_args))
        delta_model =LoraModel(backbone_model=model, modified_modules=['[r](\d).SelfAttention.[q,v,o,k]','[r](\d).EncDecAttention.[q,v,o,k]','[r](\d).DenseReluDense.w[o,i]'])
        #LoraModel(backbone_model=model, modified_modules=['[r](\d).EncDecAttention.[q,v,o,k]'])
        #delta_model = LoraModel(backbone_model=model, modified_modules=['[r](\d).DenseReluDense.w[o,i]'])
    ...
    if training_args.push_to_hub: # TODO add description here
        ...
        delta_model.save_finetuned("saved/"+repo_name,push_to_dc=False)
